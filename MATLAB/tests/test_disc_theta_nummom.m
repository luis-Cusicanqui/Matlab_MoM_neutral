...
test_name = place + "test_weak_disc_theta_1000_mom_rcx_1e0_mom";
...
%PLASMA CONSTANTS
up_amplitude_init = 1e0;
thetap_amplitude_left = 2e2;
thetap_amplitude_right = 1e2;

%NEUTRAL INITIAL CONDITION CONSTANT
amplitude_rho_high =7e0;
amplitude_rho_low = 1e0;
amplitude_u_init = 1e0;
amplitude_theta_left = 2e2;
amplitude_theta_right = 1e2;

syms v
hermite_pol = hermite(v,M(5)+1);
max_eig(1) = double(up_amplitude_init+ sqrt(thetap_amplitude_left)*max(vpasolve(hermite_pol(M(1)+2))));
max_eig(2) = double(up_amplitude_init + sqrt(thetap_amplitude_left)*max(vpasolve(hermite_pol(M(2)+2))));
max_eig(3) = double(up_amplitude_init + sqrt(thetap_amplitude_left)*max(vpasolve(hermite_pol(M(3)+2))));
max_eig(4) = double(up_amplitude_init + sqrt(thetap_amplitude_left)*max(vpasolve(hermite_pol(M(4)+2))));
max_eig(5) = double(up_amplitude_init + sqrt(thetap_amplitude_left)*max(vpasolve(hermite_pol(M(5)+2))));
...
u(k) = amplitude_u_init;
theta(k) = piecewise(k<=0.5,amplitude_theta_left,k>0.5,amplitude_theta_right);

syms u_equi(k) theta_equi(k)

u_equi(k) = up_amplitude_init;
theta_equi(k) = piecewise(k<=0.5,thetap_amplitude_left,k>0.5,thetap_amplitude_right);
